...
object $appName$ extends GMFabricServer {
  ...
  val $appName;format="camel"$Manager = new $appName$Manager
  ...
  def thrift = Some(new ThriftServer(
    Nil,
    new $appName$ThriftService($appName;format="camel"$Manager)
  ))
 ...
 */
  def rest = Some(new RestServer(
    Nil,
    Seq(new $appName$RestController($appName;format="camel"$Manager))
  ))
